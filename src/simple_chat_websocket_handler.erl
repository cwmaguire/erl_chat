-module(simple_chat_websocket_handler).
...
init(Req, State) ->
    io:format(user, "Simple Websocket handler init: Req =~n~p~n", [Req]),
    {cowboy_websocket, Req, State, #{idle_timeout => 8 * ?HOUR}}.

websocket_init(State) ->
    simple_chat_server:register_conn(),
    {ok, State}.

websocket_handle({text, Bin}, State) when is_binary(Bin) ->
    simple_chat_server:send(Bin),
    {ok, State};
...
terminate(Reason, _Req, _State) ->
    simple_chat_server:unregister_conn(Reason),
    ok.
